...
for(a in levels(as.factor(dict$patient))){
	for(b in levels(as.factor(dict$patient))){
	w <- which(dict$patient==a & dict$type=='PDAC')
	w <- c(w,which(dict$patient==b & dict$type=='PRE'))
	tds <- ds[ ,w]
	count <- nrow(tds[tds[ ,1]>0 & tds[ ,2]>0, ])
	total <- nrow(tds[tds[ ,2]>0 ,])
	pct <- round(100*(count/total),2)
	cat(paste0(as.character(a),'-',as.character(b),
