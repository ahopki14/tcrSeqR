...
pdf(paste0(path, 'number-vs-clonality.pdf'),height=6,width=6)
plot(dict[-c(1,2),'cells'],stats$Clonality,
  xlab='Input Cell Number',
  ylab='Clonality')
dev.off()

pdf(paste0(path, 'age-vs-richness.pdf'),height=6,width=6)
plot(dict[-c(1,2),'age'],stats$Richness,
  xlab='Age at Diagnosis',
  ylab='Richness')
dev.off()

pdf(paste0(path, 'age-vs-clonality.pdf'),height=6,width=6)
plot(dict[-c(1,2),'age'],stats$Clonality,
  xlab='Age at Diagnosis',
  ylab='Clonality')
dev.off()

pdf(paste0(path, 'age-vs-overlap.pdf'),height=6,width=6)
...
pdf(paste0(path, 'clonality-vs-overlap.pdf'),height=6,width=6)
plot(stats$Clonality,stats$overlap,
  xlab='Clonality',
  ylab='Overlap')
dev.off()

pdf(paste0(path, 'richness-vs-overlap.pdf'),height=6,width=6)
