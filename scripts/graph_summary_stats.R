...
for(a in seq_along(types)){
  ...
  for(b in seq_along(tstats)){
    ...
    nr <- tstats[tdict$response=='NR' ,b]
 		...
 		dir.create(paste0('/home/ahopkins/Documents/emj/ImmunoseqResults/adjuvant_study/plots/',
 		            colnames(tstats)[b],'/'),
 		            recursive=TRUE)
 		pdf(paste0('/home/ahopkins/Documents/emj/ImmunoseqResults/adjuvant_study/plots/',
 		            colnames(tstats)[b],'/',types[a],
 		            '.pdf'),
 		            width=4.5,height=8)
 		par(oma=c(1,2,1,1))	
...
nr <- ol[resp=='NR']
p <- t.test(r,nr,var.equal=TRUE)$p.value
dir.create('/home/ahopkins/Documents/emj/ImmunoseqResults/adjuvant_study/plots/Pre-Post/',
 		            recursive=TRUE)
pdf('/home/ahopkins/Documents/emj/ImmunoseqResults/adjuvant_study/plots/Pre-Post/overlap.pdf',
 		            width=4.5,height=8)
stripchart(ol ~ resp,
      at=c(1.25,1.75),
			pch=19,
