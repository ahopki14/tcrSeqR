...
for(a in seq_along(mats)){
	...
	tds$p_adj[tds$p_adj==0] <- lowest
	tds <- tds[which(tds$PDAC>0),]
	l <- max(abs(-log(tds$p_adj)))
	plots[[a]] <- ggplot(tds,aes(x=-log(p_adj),y=PDAC,color=lfc)) + 
		geom_point() +
		ylab('Tumor Counts') + 
		xlab('-log(p)') + 
		ggtitle(paste0(patients[a],'(',resp[a],')')) +
...
ggsave(g,file='~/Desktop/test.pdf',height=20)
```
