...
exp_clones <- readRDS('~/Documents/emj/ImmunoseqResults/data/neoadjuvant/exp_clones.Rds')
...
d_plot_ds <- delta_stats(plot_ds,'PRE','POST','type',names(plot_ds)[12:14],'patient',merge=TRUE)
w <- grep('Log',names(d_plot_ds))
l <- vector('list')
for(a in seq(length(w))){
  l[[a]] <- iseqr_plot_factor(d_plot_ds,names(d_plot_ds)[w[a]],'response','POST') +
...
df <- merge(plot_ds,exp_clones,by=c('patient','type'),all=TRUE)
g <- iseqr_plot_factor(df,names(exp_clones)[1],'response',type='POST') 
f <- iseqr_plot_factor(df,names(exp_clones)[1],'arm',type='POST')
